...
2. **`insert_disease_data.sql`** - 病害資訊資料插入腳本
   - 將病害資訊（從 disease_info.json）插入到 `disease_library` 表
   - 使用 `ON CONFLICT` 處理重複資料（如果已存在則更新）
   - 包含 6 種病害資訊：
   ...
   - 執行方式：`psql -U postgres -d leaf_disease_ai -f database/insert_disease_data.sql`
...
```bash
# 執行病害資訊插入腳本
psql -U postgres -d leaf_disease_ai -f database/insert_disease_data.sql
```

**注意**：此腳本使用 `ON CONFLICT` 處理重複資料，可以安全地重複執行。如果病害資訊已存在，會自動更新為最新資料。
