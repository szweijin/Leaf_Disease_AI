...
1. **`init_database.sql`** - 完整資料庫初始化腳本（**主要腳本，已合併所有遷移**）

    - 創建所有表結構
    - 創建索引
    - 創建外鍵約束
    - **已包含圖片存儲欄位**（image_data, image_data_size, image_compressed - 已棄用，保留以維持向後兼容）
    - **已包含 Cloudinary 優化**（註釋、索引、約束）
    - **已包含視圖**（user_statistics, error_statistics, api_performance_stats）
    - **已包含函數**（has_permission, log_activity, update_timestamp）
    - **已包含觸發器**（自動更新時間戳）
    - **已包含 prediction_log 表**（CNN + YOLO 流程記錄）
    - **已包含病害資訊資料**（6 種病害，自動插入）
    - **已合併所有遷移腳本**（所有遷移內容已整合到此檔案中）
...
**注意**：`init_database.sql` 已包含所有內容（表結構、視圖、函數、觸發器、圖片存儲功能、Cloudinary 優化），無需執行其他腳本。所有遷移腳本已合併到此檔案中。
...
**注意**：病害資訊資料已包含在 `init_database.sql` 中，執行初始化腳本時會自動插入 6 種病害資訊：
- 番茄早疫病 (Tomato_early_blight)
- 番茄晚疫病 (Tomato_late_blight)
- 番茄細菌性斑點病 (Tomato_bacterial_spot)
- 馬鈴薯早疫病 (Potato_early_blight)
- 馬鈴薯晚疫病 (Potato_late_blight)
- 甜椒細菌性斑點病 (Bell_pepper_bacterial_spot)

使用 `ON CONFLICT` 處理重複資料，可以安全地重複執行。如果病害資訊已存在，會自動更新為最新資料。
...
-   **users** - 用戶表
-   **detection_records** - 檢測記錄表（包含圖片存儲欄位、prediction_log_id 關聯）
-   **prediction_log** - CNN + YOLO 預測流程記錄表
...
| image_path         | VARCHAR(500) | 圖片 URL：Cloudinary URL（https://...）或本地路徑（/image/xxx） |
...
### 圖片存儲欄位（已棄用，保留以維持向後兼容）

| 欄位             | 類型    | 說明                                                         |
| ---------------- | ------- | ------------------------------------------------------------ |
| image_data       | BYTEA   | **已棄用**：圖片現在完全儲存在 Cloudinary，此欄位不再使用   |
| image_data_size  | INTEGER | **已棄用**：圖片現在完全儲存在 Cloudinary，此欄位不再使用   |
| image_compressed | BOOLEAN | **已棄用**：圖片現在完全儲存在 Cloudinary，此欄位不再使用   |
| image_path       | VARCHAR(500) | **主要欄位**：儲存 Cloudinary URL（https://...）或本地路徑 |
...
2. **圖片存儲**：
    ...
    - **圖片完全儲存在 Cloudinary**，資料庫只儲存 URL
    - `image_path` 欄位儲存 Cloudinary URL（https://...）或本地路徑（/image/xxx）
    - `image_data`、`image_data_size`、`image_compressed` 欄位已棄用但保留以維持向後兼容
    - 資料庫已添加索引和約束優化 Cloudinary URL 查詢
...
-   [database_manager.py](./database_manager.py) - 資料庫管理腳本（init/reset）
